/*	FORMATSP.C
	...
	To compile with HITEC C:
      ...
      HITEC C needs -LF flag for float formatting in printf to work!

      c formatsp.c -lf

   Do not forget the -LF flag
   for float formatting in printf to work!
...
int main()
{
   ...
   */
   ...
   /* PI is a constant, so use const keyword
     if possible to avoid compiler warning

     TODO:               
     Old school HITEC C does not support const keyword for float,
     so eventually try the use of #define instead
   */
   float mypi = 3.141592;
   double myd = 3.141592987;
